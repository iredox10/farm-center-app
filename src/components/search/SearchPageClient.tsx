...
function SearchContent({ initialProducts = allMockProducts }: { initialProducts?: any[] }) {
  ...
  const filteredProducts = useMemo(() => {
    return (initialProducts || allMockProducts)
      .filter((p) => {
        ...
        if (selectedCategories.length > 0 && !p.categoryIds.some((c: any) => selectedCategories.map((sc) => sc.toLowerCase()).includes(c))) return false;
  ...
  const toggleCategory = (cat: string) => {
    setSelectedCategories((prev) => prev.includes(cat) ? prev.filter((c: any) => c !== cat) : [...prev, cat]);
  };
  const toggleCondition = (cond: string) => {
    setSelectedConditions((prev) => prev.includes(cond) ? prev.filter((c: any) => c !== cond) : [...prev, cond]);
  };
