...
const relatedProducts: (Product & { shopName: string; imageUrl?: string })[] = [
...
export default function ProductDetailPage({ product = mockProduct, shop = mockShop, relatedProducts = [], reviewsData = initialReviews }: any) {
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white border border-outline-variant/50 rounded-3xl overflow-hidden aspect-square relative group">
              ...
              {mockImages.length > 1 && (
                <>
                  <button
                    onClick={() => setCurrentImage((p: any) => (p > 0 ? p - 1 : mockImages.length - 1))}
                  ...
                  <button
                    onClick={() => setCurrentImage((p: any) => (p < mockImages.length - 1 ? p + 1 : 0))}
