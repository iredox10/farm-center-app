...
export default function ShopStorefrontPage({ shop, products }: any) {
  ...
  const formattedProducts = products.map((p: any, idx: any) => ({
    ...p,
    id: p.$id || p.id,
  ...
  const latestIphones = formattedProducts.filter((p: any) => p.categoryIds?.includes('phones') || p.name.toLowerCase().includes('iphone'));
  const featuredPhone = latestIphones.find((p: any) => p.isFeatured) || latestIphones[0];
  const regularPhones = latestIphones.filter((p: any) => p.id !== featuredPhone?.id);
  const gamingLaptops = formattedProducts.filter((p: any) => p.categoryIds?.includes('laptops') || p.name.toLowerCase().includes('zephyrus'));
